@{
  ...
  Tokens = @(
    'NAMESPACE',
    'FILTERABLE_COLUMNS_ARRAY',
    'SEARCHABLE_COLUMNS_ARRAY',
    'DEFAULT_PER_PAGE',
    'MAX_PER_PAGE',
    'SORTABLE_COLUMNS_ARRAY',
    'JOINABLE_ENTITIES_MAP',
    'CRITERIA_HELPERS'
 )
  Content = @'
<?php
...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [[FILTERABLE_COLUMNS_ARRAY]];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [[SEARCHABLE_COLUMNS_ARRAY]];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        $x = [[SORTABLE_COLUMNS_ARRAY]];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\[[NAMESPACE]]\Definitions::class, 'softDeleteColumn')) {
            $soft = \[[NAMESPACE]]\Definitions::softDeleteColumn();
