...
- **Base** (`schema-views-<engine>.yaml`): one contract view per table, no joins. Canonical, shared by everyone.
- **Joins** (`schema-views-joins-<engine>.yaml`): multi-table analytics/metrics expected across services. Still first-party/standard.
- **Featured** (`schema-views-feature-<engine>.yaml`): shared extensions likely reused across multiple internal modules. Keep minimal.
- **Module featured** (`schema-views-feature-modules-<engine>.yaml`): per-module, domain-specific views (crypto, catalog, RBAC, replication, SLO). Owner is the module name (folder), not the underlying table.
- **External featured placeholders** (`schema-views-feature-<engine>-ext*.yaml`): reserved slots for external partners; start empty, reviewed before use.

File naming convention: `schema-views-<layer>-<engine>[suffix].yaml`, where `engine` is `mysql` or `postgres`. The generator loads all matching feature files (`schema-views-feature-<engine>*.yaml`) so module/external files are picked up automatically.
